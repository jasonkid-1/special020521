...
if ($_SESSION['userid'] != $_GET['stud_id'] and $_GET['course']) {
  header("location: ../404/404.php");
}

$q = mysqli_query($db,"SELECT *,tbl_courses.course_id, tbl_subjects_new.subj_id, tbl_departments.department_id, tbl_year_levels.year_id, tbl_semesters.sem_id FROM tbl_schedules
                                  ...
                                  LEFT JOIN tbl_semesters ON tbl_semesters.sem_id = tbl_subjects_new.sem_id
                                   where tbl_courses.course_id = '$_GET[course]' AND acad_year ='$_SESSION[active_acad]' AND tbl_schedules.semester='$_SESSION[active_sem]' order by year_abv ") or die(mysqli_error($db));
$i= 0;
while($row = mysqli_fetch_array ($q)){
  $id=$row['class_id']; 
  $subj_desc = mysqli_real_escape_string($db,$row['subj_desc']);
  $o = mysqli_query($db,"SELECT * FROM tbl_enrolled_subjects LEFT JOIN tbl_subjects_new ON tbl_subjects_new.subj_id = tbl_enrolled_subjects.subj_id where subj_desc = '$subj_desc' AND stud_id = '$_GET[stud_id]'")or die(mysqli_error($db));
