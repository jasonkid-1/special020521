...
$server = 'localhost';
    $username = 'u733437513_03132804Jaypog';
    $password = '03132804Jason';
    $dbase = 'u733437513_enrollment';
...
$pdf ->Cell(40 ,5,'',0,0);
$pdf ->Cell(25 ,5,$_SESSION['active_sem'],'B',0,0);
$pdf ->Cell(45 ,5,'Semester, Academic Year',0,0,'C');
$pdf ->Cell(53 ,5,$_SESSION['active_acad'],'B',0,'C');
...
$pdf->SetFont('Arial','','11');
$pdf ->Cell(56 ,5,$row['lastname'],'B',0,'C');//lastname
$pdf ->Cell(56 ,5,$row['firstname'],'B',0,'C');//firstname
$pdf ->Cell(56 ,5,$row['middlename'],'B',1,'C');//middlename
$pdf ->Ln(1);
$pdf ->Cell(50 ,3,'',0,0); 
$pdf->SetFont('Arial','','8');
...
$pdf ->Cell(180 ,5,$row['address'],'B',1,'C');//address
...
$pdf ->Cell(48 ,5,$row['birthplace'],'B',0,'C');//birthplace
$pdf->SetFont('Arial','','10.5');
...
$pdf->SetFont('Arial','','11');
$pdf ->Cell(70 ,5,$row['flastname'].' '.$row['ffirstname'].' '.$row['fmiddlename'],'B',0,'C');//father's name
$pdf->SetFont('Arial','','10.5');
$pdf ->Cell(29 ,5,'Name of Mother:',0,0);
$pdf->SetFont('Arial','','11');
$pdf ->Cell(70 ,5,$row['mlastname'].' '.$row['mfirstname'].' '.$row['mmiddlename'],'B',1,'C');//mother's name
...
$pdf->SetFont('Arial','','11');
$pdf ->Cell(49 ,5,$row['glastname'].' '.$row['gfirstname'].' '.$row['gmiddlename'],'B',0,'C');//guardian name
$pdf->SetFont('Arial','','10.5');
...
$sql = mysqli_query($db,"SELECT *, tbl_students.stud_id,tbl_subjects_new.subj_id,tbl_schedules.class_id FROM tbl_enrolled_subjects
    ...
    WHERE tbl_enrolled_subjects.stud_id = '$_GET[stud_id]' AND tbl_enrolled_subjects.acad_year = '$_SESSION[active_acad]' AND tbl_enrolled_subjects.semester = '$_SESSION[active_sem]' ORDER BY subj_code")or die(mysqli_error($db));
    $y = $pdf->Gety();
    $xy = 7;
...
$pdf->SetXY(10,163);
$pdf->SetFont('Arial','','9');
$sql = mysqli_query($db,"SELECT *, tbl_students.stud_id,tbl_subjects.subj_id,tbl_schedules_old.class_id FROM tbl_enrolled_subjects
    LEFT JOIN tbl_students ON tbl_students.stud_id = tbl_enrolled_subjects.stud_id
